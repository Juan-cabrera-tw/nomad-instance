...
CONSUL_JOIN=$(cat /tmp/nomad-server-addr | tr -d '\n')
...
if [ -f "$SERVER_FILE" ]; then
    ...
    CONSUL_FLAGS="-server -bootstrap-expect=1 -join=${CONSUL_JOIN} -data-dir=/opt/consul/data"
EOF
else
    echo "$SERVER_FILE doesn't exists."
    cat >/tmp/consul_flags << EOF
